...
while cap.isOpened():
    ...
    for i,cnt1 in enumerate(valid_contours):
        ...
        for j,cnt2 in enumerate(valid_contours):
            ...
            # Check if cnt1's bounding box is inside cnt2's bounding box
            if x1 >= x2 and y1 >= y2 and x1 + w1 <= x2 + w2 and y1 + h1 <= y2 + h2:
                outer_contours.append(cnt1)
    ...
    #for cnt in filter_outer_contours(valid_contours):
        (x, y, w, h) = cv2.boundingRect(cnt)
        area = cv2.contourArea(cnt)
        if 500<area<1500:
            cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
            cv2.putText(frame,"Small",(x,y),1,1,(255,255,0))
        elif 1500<area<3000:
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
            cv2.putText(frame,"Medium",(x,y),1,1,(255,255,0))
        else:
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 0, 255), 2)
            cv2.putText(frame,"Large",(x,y),1,1,(255,255,0))
